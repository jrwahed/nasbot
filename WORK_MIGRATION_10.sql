-- ============================================================================
-- WORK_MIGRATION_10.sql — ترجيع «عجل» نشطة (تصليح صغير)
--
-- الزقه كله مرة واحدة في Supabase ← SQL Editor ← Run. آمن يتكرر.
--
-- **إيه اللي حصل:**
--   الفاحص الشامل لقى بند واحد فاشل:
--       0075 · «عجل» رجعت نشطة  →  فشل — cycling لسه مقفولة
--
--   السبب مش باج في الكود — ترتيب لزق. فيه هجرتين بيكتبوا على نفس الصف:
--       0065 (دفعة ٦):  بتقفل «عجل»   ← حل مؤقت قديم
--       0075 (دفعة ٧):  بترجّعها نشطة ← بعد ما الكود اتصلّح
--   اللي يتلزق **آخر** هو اللي يكسب. فلو الدفعة ٦ اتلزقت بعد ٧، أو اتكررت
--   بعدها، «عجل» بتقفل من تاني **بالصمت** ومحدش ياخد باله.
--
-- **الملف ده بيعمل حاجتين:**
--   ١) يرجّع «عجل» نشطة دلوقتي.
--   ٢) والأهم — **المشكلة دي عمرها ما هتتكرر تاني**: 0065 نفسها اتحصّنت في
--      الكود، بقت ما تقفلش «عجل» لو حارس 0075 موجود. يعني ترتيب اللزق بطل
--      يفرق خالص. (التحصين ده في الكود، مش محتاج منك حاجة — بس لو لزقت
--      الدفعة ٦ تاني في أي وقت مستقبلًا هتشتغل صح.)
--
-- بعد ما يخلص شغّل ده:
--     select * from test_cycling_repair();
-- أو الزق `CHECK_DB.sql` تاني — المفروض يرجّع «مفيش ولا فشل».
-- ============================================================================


-- ############################################################################
-- # 20260912120000_0083_cycling_repair.sql
-- ############################################################################
