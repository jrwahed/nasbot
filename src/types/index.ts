...
export interface WhoBooked {
  ...
  total: number
  /**
   * كام واحد من الحاجزين من نفس منطقة اللي بيبص — **من غير ما يعدّه هو**.
   *
   * `null` معناها «مش عارفين»: زائر مش مسجّل، أو عضو منطقته «غير كده».
   * و`0` معناها «مفيش حد من ناحيتك» — التنتين كلام مختلف، فما يتلموش.
   */
...
export interface Person {
  ...
  photo: string
  arrived?: boolean
  /** كود المنطقة من القاعدة — بيتترجم بـ`areaFromDb` */
  areaCode?: string
  /** اللي كاتب منطقته بإيده («غير كده») — كلامه هو */
  areaLabel?: string
  /** من نفس منطقتي؟ **القاعدة** هي اللي بتقرر، مش الواجهة */
  sameArea?: boolean
}
...
export interface HostedSbota {
  ...
  status: string
  note: string
  /** العلامة اللي المجموعة تعرفه بيها في المكان — بتبان لهم مع الكشف بس */
  sign?: string
}
