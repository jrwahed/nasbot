-- ============================================================================
-- WORK_MIGRATION_11.sql — محتوى الصفحات والرسايل
--
-- الزقه كله مرة واحدة في Supabase ← SQL Editor ← Run. آمن يتكرر.
--
-- **بيحط:**
--   · ١٧ سؤال في /faq  · ٦ فقرات في /about  · ١٢ فقرة في /terms
--   · القاعدة رقم ١ بصياغتها النهائية
--   · ٢١ قالب رسايل بنبرة الإيميل
--   · ٨ رسايل «القسم مقفول»
--
-- ⚠⚠ **أهم حاجة: متغيّرات الرسايل اتصلّحت قبل ما تتحط.**
--    الكاتب بنى أرقام `{{1}}` على توقّع منطقي ونبّه إنها محتاجة مطابقة.
--    طابقناها مع `CORE` في الكود (هي اللي بتبعت القيم فعلًا) ولقينا
--    **٩ من ١١ قالب** أرقامهم كانت هتبقى غلط. أوضح مثال:
--        reminder_24h  →  {{1}} هي **اسم السبوطة** مش اسم العضو،
--                         فالإيميل كان هيقول «بكرة خروجتك يا بادل التجمع».
--    كله اتظبط. وفيه دلوقتي حارس في `npm run verify` بيفشل البناء لو حد
--    حط متغيّر مش موجود.
--
-- ⚠ **ولقينا باجين كانوا موجودين قبل المحتوى ده:**
--    · `transfer_received` كان فيه `{{1}}` وهو مش مربوط بأي قيمة — الإيميل
--      كان بيوصل **«وصلنا تحويلك لـ .»** بفراغ. اتصلّح.
--    · `mutual_match` كان بياخد رابط الشات ومش مستعمله — العضو بيعرف إن
--      الشات اتفتح ومش لاقي لينك. اتصلّح.
--
-- ⚠ **صفحة الشروط هتفضل مقفولة عن الناس** لحد ما تعمل تلات حاجات:
--    ١) تملا «مدة صلاحية الرصيد» في فقرة الإلغاء.
--    ٢) تملا «وسيلة التواصل» في آخر فقرة.
--    ٣) محامي مصري يراجع فقرة «حدود المسؤولية» والصفحة كلها.
--    وبعدين افتحها من /admin/content ← «صفحات الموقع» ← الشروط.
--    ولحد ساعتها رابطها مش بيظهر في ذيل الموقع خالص.
--
-- ⚠ وفقرة **«احنا مين»** في /about مقفولة كمان — محتاجة كلامك انت،
--    وما اخترعناش فريق ولا تاريخ.
--
-- بعد ما يخلص:
--     select * from test_brief_content();
-- أو الزق `CHECK_DB.sql` — المفروض «مفيش ولا فشل».
-- ============================================================================


-- ############################################################################
-- # 20260912130000_0084_brief_content.sql
-- ############################################################################
