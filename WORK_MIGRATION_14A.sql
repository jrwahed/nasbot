-- ============================================================================
-- WORK_MIGRATION_14.sql — «دليل السبوطات» يحل محل بيانات العرض
--
-- ⚠⚠ **الملف ده جزئين، والجزء الأول لازم يخلص قبل ما التاني يبدأ.**
--
--    الجزء الأول (`0088`) بيضيف قيم `enum` جديدة، وبوستجرس ما بيسمحش
--    تستخدم قيمة enum في نفس المعاملة اللي أضافتها — ومحرر SQL بيشغّل
--    الملف كله كمعاملة واحدة. فلو لزقت الاتنين مرة واحدة، الجزء التاني
--    هيقع بـ «invalid input value for enum template_kind_t».
--
--    فالزق **WORK_MIGRATION_14A.sql** الأول، استنى «Success»،
--    وبعدين **WORK_MIGRATION_14B.sql**.
--
-- بعديهم الزق `CHECK_DB.sql` — المفروض «✅ مفيش ولا فشل».
--
-- ⚠ **اللي هيتمسح:** كل بيانات العرض — السبوطات والحجوزات والكباتن
--    والأماكن (ما عدا أماكن الشغل) والقوالب القديمة والأعضاء الوهميين.
--    **مش هيتمسح:** حساباتك في اللوحة، القالب العام `khroga-men-3odw`،
--    وطبقة الشغل كلها.
--
-- ⚠ **الصور مش جوه الملف ده.** القوالب الـ٨٥ بتتحط من غير صور — ترفعها
--    من `/admin` ← القوالب ← «صور السبوطة». ونفس الحكاية للأسعار
--    والمواعيد: الدليل متعمّد إنه مفيهوش أرقام.
-- ============================================================================

-- ############################################################################
-- # 20260912200000_0088_template_seo_fields.sql
-- ############################################################################
