-- ============================================================================
-- WORK_MIGRATION_23.sql — العضو يعرف إنه اتقبل أو اترفض
--
-- ⚠ بوابة `0101` بتقبل وبترفض، بس العضو ما كانش بيعرف — كان لازم يفتح
--    الموقع بنفسه ويلاقي الحالة اتغيّرت. حد بيستنى موافقة ومحدش بيكلمه
--    بيفتكر إنه اترفض.
--
-- قالبين جداد بيمروا من نفس مصرف الإشعارات، والإشعار بيتبعت من **جوّه**
-- `fn_gate_decide` — مش من الواجهة: القبول ممكن يحصل من أي مكان.
--
-- ⚠ **الملف ده بيعيد كتابة `fn_gate_decide` من `0101`.** الزقه **بعد**
--    `WORK_MIGRATION_22.sql` — لو اتلزقوا بالعكس، النسخة القديمة (اللي من
--    غير إشعار) هي اللي هتكسب (درس تالت).
--
-- بعده شغّل: select * from test_gate_notify();     — الخمسة «نجح»
--            select * from test_entry_gate();      — السبعة لسه «نجح»
-- ============================================================================

-- ############################################################################
-- # 20260920160000_0102_gate_notify.sql
-- ############################################################################
