...
insert into copy_strings (key, value_ar, screen, context_ar) values
  ...
  ('host.mine.sign',      'العلامة اللي هيعرفوك بيها', 'خروجاتي', 'خانة العلامة لصاحب الخروجة'),
  ('host.mine.signPh',    'هقعد على الترابيزة اللي جنب الشباك، وهيكون معايا كتاب أصفر', 'خروجاتي',
   'مثال في الخانة'),
  ('host.mine.signHint',  'بتوصلهم مع كشف المجموعة — قبل الخروجة بيوم. سطر واحد يخلّيهم يلاقوك من غير ما يلفوا.', 'خروجاتي',
   'شرح تحت الخانة'),
  ('host.mine.signSave',  'احفظ العلامة', 'خروجاتي', 'زرار الحفظ'),
  ('host.mine.signSaved', 'اتحفظت ✓', 'خروجاتي', 'بعد الحفظ')
on conflict (key) do update
  set value_ar   = excluded.value_ar,
      screen     = excluded.screen,
      context_ar = coalesce(excluded.context_ar, copy_strings.context_ar);
