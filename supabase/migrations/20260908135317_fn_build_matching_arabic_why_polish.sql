-- تظبيط النص اللي الأعضاء بيقروه (why_ar) علشان العدد يتقال صح بالمصري،
-- وكمان اسم اللي شغّل المطابقة بدل ما نكتب الـ uuid في جدول التشغيلات.
...
begin
  ...
  for v_i in 1..v_k loop
    ...
    -- بالمصري والعدد بيتقال صح (واحد/اتنين/جمع) مش أرقام ناشفة.
    v_why := 'اخترناكم مع بعض: '
             || (case v_cnt when 2 then 'اتنين' else format('%s ناس', v_cnt) end);
    ...
    if not v_s.girls_only and v_gg > 0 and v_gg < v_cnt then
      v_why := v_why || ' · '
        || (case v_gg when 1 then 'بنت واحدة' when 2 then 'بنتين'
                      else format('%s بنات', v_gg) end)
        || ' و'
        || (case v_cnt - v_gg when 1 then 'ولد واحد' when 2 then 'ولدين'
                              else format(' %s ولاد', v_cnt - v_gg) end);
    end if;

    if v_gs = 1 then
      v_why := v_why || ' · فيكم واحد بيبدأ الكلام — يعني القعدة مش هتسكت.';
    elsif v_gs = 2 then
      v_why := v_why || ' · فيكم اتنين بيبدأوا الكلام — يعني القعدة مش هتسكت.';
    elsif v_gs > 2 then
      v_why := v_why || format(' · فيكم %s بيبدأوا الكلام — يعني القعدة مش هتسكت.', v_gs);
    else
  ...
  -- ------------------------------------------------------------- التسجيل
  -- اسم اللي شغّلها بيبان في جدول التشغيلات، فالاسم أنفع من الـ uuid
