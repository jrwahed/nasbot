-- ============================================================================
-- WORK_MIGRATION_34.sql — مسح صور الأعضاء بصلاحية
--
-- فتحنا باب رفع الصور للأعضاء في الحزمة اللي فاتت (`0113`). الملف ده
-- بيظبّط **مين يمسحها**.
--
-- سياسة `photos_admin` القديمة شكلها `for all using fn_is_admin()` —
-- و`fn_is_admin` معناها «أي صف نشط في `admin_users`»، يعني حتى دور
-- `support` كان يقدر يمسح صور الناس. بقت الصلاحية `sbotat.edit`.
-- (نفس الغلطة اللي اتصلّحت على الأسعار في `0056`.)
--
-- والقراية سايبينها لأي حساب لوحة عن قصد، علشان صفحة الإشراف الجديدة
-- `/admin/photos` يشوفها الكل والمسح للي من حقه.
--
-- والعضو لسه بيمسح **صوره هو** زي ما هي.
--
-- بعده شغّل:  select * from test_photo_admin_perm();   — ٣ صفوف «نجح».
-- ============================================================================

-- ##########################################################################
-- # 20260921170000_0115_photos_admin_perm.sql
-- ##########################################################################
