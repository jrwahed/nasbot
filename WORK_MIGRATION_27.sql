-- ============================================================================
-- WORK_MIGRATION_27.sql — مفتاح ميزة للكباتن
--
-- المشكلة: `/shoghl` و`/s/mystery` و`/captains` مبنيين بالكامل و**صفر
-- بيانات**. الزائر الأول بيلاقي صفحات فاضية فالمشروع باين متوقف.
--
-- `work_sbota` و`mystery` مفاتيحهم موجودة من زمان. **`captains` مالوش
-- مفتاح خالص** — الملف ده بيضيفه، وبس.
--
-- ⚠ **الملف ده ما بيقفلش حاجة.** بيدّيك الزرار بس. القفل قرار من
--    `/admin/settings` ← مفاتيح المزايا.
--
-- وبعد اللزق، الكود المنشور بيخلّي القفل يخفي **الصفحة والرابط اللي
-- بيوديها** مع بعض — من القايمة والذيل. حارس `check:nav` بيفشل البناء لو
-- حد ساب رابط لصفحة ورا مفتاح من غير نفس المفتاح.
--
-- بعده شغّل:  select * from test_section_flags();   — التلاتة «نجح».
--
-- ولو عايز تخفي الأقسام الفاضية دلوقتي:
--   /admin/settings ← مفاتيح المزايا ← اقفل «سبوطة الشغل» و«السبوطة
--   الغامضة» و«صفحة الكباتن». وافتحهم تاني أول ما يبقى فيهم بيانات.
-- ============================================================================

-- ##########################################################################
-- # 20260921100000_0108_captains_flag.sql
-- ##########################################################################
