-- ============================================================================
-- WORK_MIGRATION_33.sql — الفيد «اللي بيحصل»
--
-- العضو بيخرج مرة وبيختفي، ومفيش حاجة بتوريه إن النادي **حي**.
-- الفيد بيقول له: فيه خروجات بتحصل، وفيه ناس بترجع.
--
-- 🔴 **قرارين، والاتنين محطوطين في القاعدة مش في الواجهة:**
--    1. الفيد مقفول على **اللي خرج معانا مرة على الأقل**. الزائر والعضو
--       اللي عامل حساب وبس بياخدوا فيد **فاضي**.
--    2. الفيد طبقتين: **خروجاتك انت** بصورها وبرابطها · و**باقي الخروجات**
--       كرت من غير صور ولا أسامي («٦ راحوا ورشة فخار السبت»).
--
--    الطبقة التانية دي هي اللي بتمنع الصفحة تبقى بروفايلات عامة. الدالة
--    نفسها **مش بترجّع** الصور للي مكانش في الخروجة — مش الواجهة اللي
--    بتخبّيها.
--
-- ومعاه مفتاح ميزة `feed` — تقدر تقفله من `/admin/settings` وقتما تحب،
-- والرابط بيختفي من القايمة معاه.
--
-- بعده شغّل (بالترتيب):
--   select fn_test_seed_up();
--   select * from test_feed();      -- ٥ صفوف، كلهم «نجح»
--   select fn_test_seed_down();
-- ============================================================================

-- ##########################################################################
-- # 20260921160000_0114_feed.sql
-- ##########################################################################
