-- ============================================================================
-- WORK_MIGRATION_19.sql — العنوان يرجع للي المالك كتبه · ورسالة واتساب جاهزة
--
-- تلات حاجات في ملف واحد، كلها من نفس الجذر: لما اتشالت قايمة الأماكن من
-- اللوحة وبقى المالك بيكتب المكان بإيده، القاعدة والإيميلات فضلوا بيدوّروا
-- على صف في `venues` مش موجود.
--
--   ١) `fn_sbota_address` كانت بترجّع **ولا صف** لسبوطة نسبوط — يعني حد دافع
--      ومش شايف العنوان خالص. دلوقتي بتقرا اللي المالك كتبه على السبوطة.
--   ٢) `booking_confirmed` بقى فيه رابط الحجز — زرار الإيميل كان رايح على
--      `/sbota/<slug>`، مسار مش موجود في الموقع أصلًا (404).
--   ٣) قالب جديد `whatsapp_booking` — الرسالة اللي اللوحة بتفتح بيها واتساب
--      للحاجز بعد اعتماد تحويله. بتتعدّل من `/admin/notifications`.
--
-- بعده شغّل: select * from test_address_back();  — الخمسة لازم «نجح».
-- ============================================================================

-- ############################################################################
-- # 20260913120000_0098_address_back_and_wa.sql
-- ############################################################################
